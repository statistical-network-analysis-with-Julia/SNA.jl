...
"""
    density(net; diag=false, discount_bipartite=false, missing=:error) -> Float64
...
single-counted, so a loop-free undirected network has `ne(net) / (n(n-1)/2)`.
Loops are excluded unless `diag=true`. Two-mode density includes all actor
pairs unless `discount_bipartite=true` restricts the denominator to cross-mode
pairs. With `diag=true`, undirected density follows Networks' `n(n+1)/2`
possible-dyad denominator, not R sna's symmetric-matrix `n²` convention.
...
"""
function density(net::AbstractNetwork; diag::Bool=false, discount_bipartite::Bool=false,
                 missing::Symbol=:error)
    policy = missing  # local alias; `missing` here is the kwarg, not `Base.missing`
    require_observed(net, policy; context="density")
    # Networks.network_density guards itself now (panel 2026-09, item 4), so the
    # policy the caller chose here is forwarded rather than re-checked as :error.
...
function efficiency(net; missing::Symbol=:error)
    ...
    comps = Graphs.weakly_connected_components(_graph(net))
...
"""
    component_dist(net; connected=:strong, missing=:error) -> Vector{Int}
...
`Networks.require_observed`).
"""
function component_dist(net; connected::Symbol=:strong, missing::Symbol=:error)
    policy = missing  # local alias; `missing` here is the kwarg, not `Base.missing`
    require_observed(net, policy; context="component_dist")
    comps = components(net; connected, missing=policy)
