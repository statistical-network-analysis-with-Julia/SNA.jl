...
SNA.jl is a Julia port of the R `sna` package (StatNet collection). It provides social network analysis tools built on top of the `Networks` package and `Graphs.jl`.

## Common Commands

```bash
# Install dependencies (must resolve Networks first — see note below)
...
- Measures follow R `sna` conventions exactly where a counterpart exists; full-vector sna 2.8 fixtures (Sampson samplike, Padgett flomarriage, directed, two-mode and weighted examples) are generated by `test/fixtures/r/sna_reference.R` and loaded with `Networks.load_golden`
...
- Components use `connected=:strong`; cutpoints use directed strong articulation
  ...
  bridges unless `min_size=2`. Closeness defaults to `cmode=:sna`.
...
- QAP RNGs are seeded per replicate before threading, and permutations preserve
  two-mode actor partitions. Regression covariance/intervals assume iid dyads;
  QAP p-values are a separate inference layer. The shared Networks optimizer,
...
checks complete and quasi-complete separating rays in the final coefficients
and next Newton direction. Floating arithmetic only proposes a ray; rational
arithmetic corrects its boundary constraints and verifies every signed margin
on the exact Float64 input values. Absence of a found certificate is not an
exhaustive LP proof of overlap. Do not replace this with a coefficient cutoff.
QAP fails if any requested replicate is nonidentified; it does not discard
failed draws. `_qap_replicates` reports the first failing index identically in
serial and threaded runs. Sparse binary QAP examples need to acknowledge this;
use an appropriate continuous explanatory covariate for a runnable QAP example,
not a convenient seed or fewer draws that happens to avoid the failure.
