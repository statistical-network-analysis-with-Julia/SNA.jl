...
With `cmode=:sna` (default), an actor scores zero if any other actor is
unreachable. `cmode=:component` requests the previous reachable-component
scaling from Graphs.jl. A singleton has undefined closeness (`NaN`).
...
# Tolerance for detecting a repeated leading eigenvalue
ec = eigenvector_centrality(net; tol=1e-8)
```
...
SNA.jl computes this with a direct eigen solve. A repeated leading eigenvalue
produces a warning because the score is not unique; bipartite ±eigenvalue
pairs do not cause power-iteration oscillation.

### Parameters

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `max_iter` | `Int` | `1000` | Accepted for compatibility; direct solve does not iterate |
| `tol` | `Float64` | `1e-10` | Leading-eigenvalue multiplicity tolerance |
...
```julia
# A small attenuation within the walk-series convergence region
bp = bonacich_power(net; exponent=0.05)
...
Katz centrality counts incoming walks, including a baseline of $\beta$
for each actor, with longer walks attenuated by $\alpha$.
...
$$C_K(i) = \beta \sum_{k=0}^{\infty} \sum_j \alpha^k (A^k)_{ji}$$

In matrix form:

$$\mathbf{c} = (I - \alpha A^T)^{-1} \beta\mathbf{1}$$
...
| `β` | `Float64` | `1.0` | Baseline score per actor |
| `normalized` | `Bool` | `false` | Scale to unit L2 norm |

### Interpretation

- **High Katz centrality**: The vertex receives many incoming walks
...
Centrality uses binary ties by default. Degree, eigenvector, Bonacich, Katz,
and flow betweenness accept `ignore_eval=false, attr=:weight`; all present
edges must carry finite numeric values of that attribute. These routines
ignore self-loops unless `diag=true`. Flow and spectral centralities require
nonnegative weights. Shortest-path centralities currently use binary paths.

Both two-mode network representations use the square adjacency of all actors,
including reverse-direction arcs. Dense spectral solves require O(n²) storage
and O(n³) work; flow betweenness is substantially more expensive and suits
moderate networks.
