...
|-----------|------|---------|-------------|
| `connected` | `Symbol` | `:strong` | `:weak` (ignore direction) or `:strong` (mutual reachability); legacy `mode` is accepted |
...
A clique is a maximal complete subgraph: every vertex in the clique is
connected to every other vertex. Directed cliques require mutual arcs by
default (`symmetrize=:strong`); `symmetrize=:weak` accepts either direction.
...
- Bicomponents require at least three vertices by default, as in R
  `bicomponent.dist`. Use `min_size=2` to include bridges and partition the
  full edge set. Directed inputs require mutual arcs (`symmetrize=:strong`);
  use `symmetrize=:weak` for an arc in either direction.
