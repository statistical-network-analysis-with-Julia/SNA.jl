...
Equivalence routines use the square adjacency of all actors, including both
modes of a two-mode network. Binary ties are the default;
`ignore_eval=false, attr=:weight` selects finite numeric edge values for
structural equivalence and its clustering/blockmodeling wrappers. Weighted
regular equivalence is unsupported and raises an explanatory error. Structural profiles include
all incoming and outgoing positions, including self and mutual positions.
This convention and the iterative regular-equivalence similarity are explicit
choices; these functions are not the complete R `sedist` algorithm catalogue.
