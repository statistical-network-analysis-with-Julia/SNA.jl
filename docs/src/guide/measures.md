...
## Edge cases and two-mode density

`density(net)` excludes self-loops; `diag=true` includes them using the
Networks possible-dyad convention. This differs from R `sna::gden(diag=TRUE)`
for an undirected looped graph: Networks counts each loop once among
`n(n+1)/2` dyads, while `sna` sums the symmetric matrix over `n²` entries.
With `diag=false` they agree. Two-mode density includes all actor pairs by
default; `discount_bipartite=true` restricts the denominator to cross-mode pairs.

Empty edgewise and non-null reciprocity are `NaN`. Transitivity with no
qualifying two-path is `1.0`, following R `sna` 2.8's explicit convention.
Component sizes default to strong directed connectivity; request
`component_dist(net; connected=:weak)` to ignore direction.
